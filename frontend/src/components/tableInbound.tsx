...
export default function InboundTable({ order }: Props) {
  ...
  return (
    <div className={'flex flex-col gap-4 rounded-xl mt-4 max-w-[400px] bg-[#292C2D] p-4 shadow-sm'}>
      ...
      <section className='flex w-full flex-col gap-2'>
        ...
        <aside className='flex flex-col max-h-[100px] min-h-[100px] overflow-auto gap-2'>
          {order.inbound_order_items.map(function (orderItem: InboundOrderItem, index: number) {
            return (
      ...
      <section className={'flex flex-col gap-4'}>
        <header className='flex items-center justify-between'>
          ...
          <p className='text-[16px] font-light leading-[24px] text-white'>
            $
            {order.inbound_order_items.reduce(function (total: number, amount: InboundOrderItem) {
