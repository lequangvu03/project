...
export const useGetINboundOrdersQuery = function ({ page, limit }: { page: number; limit: number }) {
  return useQuery({
    queryKey: ['INBOUND_ORDERS', page, limit],
    queryFn: async () => {
      return await inboundOrderService.getInboundOrders({ page, limit })
    }
  })
}
